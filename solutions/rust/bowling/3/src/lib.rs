...
impl BowlingGame {
    ...
    fn fill_balls_complete(&self) -> bool {
        (self.frames.last().unwrap().frame_type.unwrap() == FrameType::Open
            ...
            || self
                .fill_balls
                .as_ref()
                .is_some_and(|fill_balls| fill_balls.is_complete())
    }
    ...
    pub fn roll(&mut self, pins: u16) -> Result<(), Error> {
        ...
        if let Some(fill_balls) = &mut self.fill_balls {
            return fill_balls.roll(pins);
        }
    ...
    pub fn score(&self) -> Option<u16> {
        ...
        self.frames.iter().for_each(|frame| {
            ...
            if prev_prev_frame_type
                .is_some_and(|frame_type: FrameType| frame_type == FrameType::Strike)
            {
                score += frame.rolls[0];
            }
        ...
        if let Some(fill_balls) = & self.fill_balls {
            if prev_prev_frame_type
                .is_some_and(|frame_type: FrameType| frame_type == FrameType::Strike)
            {
                score += fill_balls.rolls[0];
            }

            score += fill_balls.rolls.iter().sum::<u16>();
        }
