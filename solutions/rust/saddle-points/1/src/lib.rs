...
pub fn find_saddle_points(input: &[Vec<u64>]) -> Vec<(usize, usize)> {
    ...
    for (row, row_data) in input.iter().enumerate() {
        ...
        for (col, col_data) in row_data.iter().enumerate() {
            cols.entry(col).or_insert_with(Vec::new).push(*col_data);
            if *col_data == *max_row_data {
                max_cols.entry(row).or_insert_with(Vec::new).push(col);
            }
        }
    }
    ...
    cols.iter().for_each(|(col, col_data)| {
        ...
        col_data.iter().enumerate().for_each(|(row, val)| {
            if *val == *min_col_data && max_cols.entry(row).or_insert_with(Vec::new).contains(col) {
                output.push((row, *col));
            }
        });
    });
